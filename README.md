...
## Learnings:
1. In `/src/app/` folder which folder we create with `page.tsx` file, then the name of folder becomes the route/path name using which we can access the page. This is how easily the routing is performed in Next.js. For eg: if inside `app/` you've created `admin/` folder with `page.tsx` file, then hitting the URL `http://localhost:3000/admin` will be rendering the `page.tsx` file in the client-side.
...
7. To allow images from specific external paths/sources (like Cloudinary, Unsplash, or other cloud providers) and block all others, we can specify them as "trusted sources" by mentioning them as a `URL instance` or `RemotePattern` type of Object in `images.remotePatterns` in [Next.js config file](https://github.com/VinayNoogler000/Music-Academy-Next.js/blob/main/next.config.ts). You can learn out more about it [here (official documentation)](https://nextjs.org/docs/app/api-reference/components/image#remotepatterns).
8. Learnt to add Shadcn UI library components on my own via "manual" installation (instead of command to make the process challenging). The component which I added was a [Toast](https://github.com/VinayNoogler000/Music-Academy-Next.js/blob/12e51961dd19829399b33f7b153c688362a1876c/src/components/ui/sonner.tsx) component in [Contact Us](https://github.com/VinayNoogler000/Music-Academy-Next.js/blob/12e51961dd19829399b33f7b153c688362a1876c/src/app/contact/page.tsx) page to give UI feedback to the user that the message has been sent successfully (even though there are no functionality implemented to send message).
